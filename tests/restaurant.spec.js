...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    returnObjectValue = objetoRetornado[(Object.keys(objetoRetornado).find((x) => x === 'fetchMenu'))];
    expect(typeof returnObjectValue).toBe('function');
    // ```
    ...
    // ```
    const randomValue = createMenu('x')
    expect(randomValue.fetchMenu()).toBe('x');
    ...
    // ```
    const checkConsumtionIsEmpty = createMenu('');
    expect(checkConsumtionIsEmpty.consuption).toEqual([]);
